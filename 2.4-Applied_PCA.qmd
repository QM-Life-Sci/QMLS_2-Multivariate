...
MM <- read_excel("Data/mammals.xlsx", na = "NA") |>
  ...
  dplyr::select(order, family, genus, species, adult_body_mass_g, adult_head_body_len_mm, diet_breadth, gestation_len_d, habitat_breadth, litter_size,  neonate_body_mass_g, sexual_maturity_age_d) |>
...
mod <- lm(maturity_age ~ adult_mass + body_length + diet_breadth + gest_length + habitat_breadth + litter_size + neonate_mass, data = MM)
...
mod <- lm(maturity_age ~ adult_mass + diet_breadth + gest_length + habitat_breadth + litter_size, data = MM)
...
mod <- lm(maturity_age ~ (adult_mass + diet_breadth + gest_length + habitat_breadth + litter_size)^2, data = MM)
...
MM |> 
  ...
  geom_segment(x = MM$adult_mass, xend = MM$adult_mass, y = MM$maturity_age, yend = MM$ypred1, color = "firebrick") +
...
- The squared correlation coefficient between predicted and observed Y values
...
1-(sum(mod1$residuals^2)/sum((MM$maturity_age-mean(MM$maturity_age))^2))

sum((mod1$fitted.values-mean(MM$maturity_age))^2)/sum((MM$maturity_age-mean(MM$maturity_age))^2)
...
mod2 <- lm(maturity_age ~ adult_mass + body_length + diet_breadth + gest_length + habitat_breadth + litter_size + neonate_mass, data = MM)

mod3 <- lm(maturity_age ~ (adult_mass + diet_breadth + gest_length + habitat_breadth + litter_size)^2, data = MM)
...
> How does land use influence pollinator abundance? [fixme - cite]
https://onlinelibrary.wiley.com/doi/full/10.1002/ece3.4087
