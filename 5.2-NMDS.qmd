...
- Does not maximize the variability associated with individual axes of the ordination
    ...
    - Plots and be inverted, rotated, etc. for interpretation 


## General method

1. Define original data points in multidimensional space based on distances between sites
2. Specify the number of reduced dimensions you want (often 2)
...
- Describes the difference between the distance values from multidimensional space calculated from the original distance matrix and the distance between points in the reduced dimension representation
    - How far the distances in the new configuration are from being proportional to the original distances
...
- No guarantee of finding the best solution
    ...
    - "Non-linear optimization is a hard task"
- Use many random start values and compare final stress as goodness-of-fit
...
Interpretation is *relative ordering* points
...
- Note mixture of categorical and continuous variables
...
ordisurf(Zoo_k2, Env$surface_temp)
```
