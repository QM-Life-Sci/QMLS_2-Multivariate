...
- Goal of PERMANOVA is decision-making (aka hypothesis testing)
- What is the probability the observed patterns are only the result of sampling error?
- PERMANOVA is applicable to any distance measure between categorical groups
...
1. SS for each variable to create a matrix of distances (aka differences)
...
ssa <- MM.dat %>% 
  ...
  geom_point() +
  geom_point(aes(x=mean(y1),y=mean(y2)),col='gray50',size=4) +
  geom_point(aes(meany1,meany2),size=4)+geom_segment(aes(x=mean(y1),y=mean(y2), xend=meany1,yend=meany2),color='black') +
  theme(legend.position = "none") +
...
- Under the null hypothesis (aka no real group differances), observations are random draws from a common population
