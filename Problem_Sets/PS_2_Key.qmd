...
# FIXME
fm <- lm(
  N_Species ~ Area + Elevation + Dist_to_Ecuador + Dist_to_Island,
  data = MM
)
summary(fm)
```
...
fm0 <- lm(
  N_Species ~ Area + Elevation + Dist_to_Ecuador + Dist_to_Island - 1,
  data = MM
)
coef(fm0)
...
```{r}
#FIXME
DD <- read_csv("../Data/Santangelo_familyMean.csv", show_col_types = FALSE)
glimpse(DD)
