...
tt <- table(Observed = BB$sex_survived, Predicted = predict(Bump_lda)$class)
...
tree <- rpart(sex_survived ~ ., data = BB, control = rpart.control(cp = 0.01))
printcp(tree)

best <- tree$cptable[which.min(tree$cptable[, "xerror"]), "CP"]
pruned_tree <- prune(tree, cp = best)

prp(pruned_tree, faclen = 0, extra = 1, roundint = FALSE, digits = 3)
