...
IDs <- str_split(flist, pattern = "/", simplify = TRUE)[, 5] |> 
...
3D arrays like this -- where the first dimension is the landmarks in sequence, the second dimension is the X, Y, (and Z) columns, and the third dimension is the specimens -- are the standard data format for carrying out geometric morphometric analyses in R using the [`geomorph` package](https://cran.r-project.org/package=geomorph).
