...
![](https://www.github.com/QM-Life-Sci/QMLS_2-Multivariate/raw/main/Images/fly_wing.png){fig-align="center" fig-alt="A photograph of a fruit fly wing with landmarks."}
...
# FIXME
IMG_1573 <- read_csv(
  "../Data/wing_landmarks//IMG_1573_LM.csv",
...
centroid_size_2 <- function(MM) {
  centroid <- colMeans(MM)
  dists <- apply(MM, MARGIN = 1, FUN = function(x) {
...
wings <- map(
  .x = flist,
  .f = function(.x) {
    MM <- read_csv(.x, show_col_types = FALSE) |>
      select(-Landmark) |>
      as.matrix()
    MM <- MM / 1498
  }
...
sigma <- rbind(c(1.00, 0.25, 0.50), c(0.25, 1.00, 0.75), c(0.50, 0.75, 1.00))
sigma
```
...
plot_ly(
  ...
  type = "scatter3d",
  mode = "markers",
  marker = list(size = 3, color = "navy", opacity = 0.25)
)
