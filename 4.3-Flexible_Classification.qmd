...
SD <- data.frame(Group = factor(rep(LETTERS[1:3], each = n)),
                 G)

ggplot(SD, aes(X1, X2, color = Group)) +
  geom_point(size = 3) +
  coord_equal() +
...
split <- initial_split(SD, strata = Group, prop = 0.75) 
...
Grid <- crossing(X1 = seq(min(SD$X1), max(SD$X1), length.out = 100),
                 X2 = seq(min(SD$X2), max(SD$X2), length.out = 100))
Grid <- Grid |> 
