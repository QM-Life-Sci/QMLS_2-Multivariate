...
![](./Images/Darlingtonia.png){fig-align="center", fig-alt="Photo of a pitcher plant"}
...
```{r}
Pmod <- manova(
  cbind(Height, Mouth_Diam, Tube_Diam, Keel_Diam) ~ Site,
  data = PP
)
...
ll$scaling[1] *
  MM$coefficients[1, 1] +
  ll$scaling[2] * MM$coefficients[1, 2] +
  ll$scaling[3] * MM$coefficients[1, 3]

ll$scaling[1] *
  MM$coefficients[2, 1] +
  ll$scaling[2] * MM$coefficients[2, 2] +
  ll$scaling[3] * MM$coefficients[2, 3]
```
...
set.seed(140592)
nn <- 25
sigma <- rbind(c(1.00, 0.75, 0.75), c(0.75, 1.00, 0.75), c(0.75, 0.75, 1.00))

MM <- rmvnorm(n = nn * 2, mean = c(0, 1, 2), sigma = sigma)
ss <- 2
MM.dat <- tibble(
  y1 = MM[, 1] + c(rnorm(nn, ss), rnorm(nn, 1, ss)),
